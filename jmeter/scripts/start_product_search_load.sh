...
sed -i '' 's+afriex_system_under_test+'"${SYSTEM_UNDER_TEST}"'+gi' api/search/productSearch.jmx
echo
numberOfTests=$(aws s3 ls s3://afriex-jmeter-reports/importexport/ --summarize  | wc -l)
numberOfTests=$((numberOfTests))
context="productsearch-$numberOfTests"
...
startTestCmd="curl -X POST -F 'jmeter=@api/search/productSearch.jmx' -F 'context=$context' -F 'numberOfNodes=1' -F 'xmx=1' -F 'xms=1' -F 'cpu=1' -F 'ram=1' -F 'maxMetaspaceSize=768' $host:1323/start-test"
echo $startTestCmd
eval "$startTestCmd"
sed -i '' 's+'"${SYSTEM_UNDER_TEST}"'+afriex_system_under_test+gi' api/search/productSearch.jmx
